...
class Program:
    ...
    def getFavoritePrograms(cls, data):
        query = 'SELECT * from favoriteprograms LEFT JOIN universityprograms on favoriteprograms.programs_id = universityprograms.id WHERE favoriteprograms.users_id = %(user_id)s;'
        results = connectToMySQL(cls.db_name).query_db(query, data)
        FavoritePrograms = []
        for row in results:
            FavoritePrograms.append(row)
        return FavoritePrograms
